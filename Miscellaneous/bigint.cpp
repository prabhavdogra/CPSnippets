...
struct BigInt {
    ...
    void __internal_add(const BigInt &v) {
        ...
        for (int i = 0, carry = 0; i < (int)std::max(a.size(), v.a.size()) || carry;
             ++i) {
    ...
    void fft(std::vector<std::complex<double>> &a_, bool invert) const {
        int n = (int)a_.size();
        ...
        for (int len = 2; len <= n; len <<= 1) {
            ...
            for (int i = 0; i < n; i += len) {
                ...
                for (int j = 0; j < len / 2; ++j) {
                    std::complex<double> u = a_[i + j];
                    std::complex<double> v = a_[i + j + len / 2] * w;
                    a_[i + j] = u + v;
                    a_[i + j + len / 2] = u - v;
                    w *= wlen;
                }
    ...
    void multiply_fft(const std::vector<int> &a_, const std::vector<int> &b,
                      std::vector<int> &res) const {
        std::vector<std::complex<double> > fa(a_.begin(), a_.end());
        std::vector<std::complex<double> > fb(b.begin(), b.end());
        int n = 1;
        while (n < (int)std::max(a_.size(), b.size())) n <<= 1;
        n <<= 1;
        fa.resize(n);
        fb.resize(n);
    ...
    BigInt mul_karatsuba(const BigInt &v) const {
        ...
        while (a_.size() < b_.size()) a_.push_back(0);
        while (b_.size() < a_.size()) b_.push_back(0);
        while (a_.size() & (a_.size() - 1)) a_.push_back(0), b_.push_back(0);
        vll c = karatsubaMultiply(a_, b_);
        BigInt res;
        res.sign = sign * v.sign;
        long long carry = 0;
...
signed main() {
    BigInt x = pow((BigInt)2, (BigInt)70000);
    cout << x;
}
